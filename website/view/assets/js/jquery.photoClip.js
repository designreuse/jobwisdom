...
(function(root, factory) {
...
}(this, function(jQuery, IScroll, Hammer) {
	"use strict";

	jQuery.fn.photoClip = function(option) {
		...
		jQuery.extend(defaultOption, option);
	...
	function photoClip(container, option) {
		...
		if (!jQueryfile.length) return;

		var jQueryimg,
			imgWidth, imgHeight, //图片当前的宽高
			imgLoaded; //图片是否已经加载完成

		jQueryfile.attr("accept", "image/*");
			...
			if (!/image\/\w+/.test(this.files[0].type)) {
				...
				fileReader.onload = function(e) {
					...
					if (kbs > 1024) {
						...
						var jQuerytempImg = jQuery("<img>").hide();
						jQuerytempImg.load(function() {
							...
							jQuerytempImg.appendTo(document.body);
							var realityHeight = this.naturalHeight;
							jQuerytempImg.remove();
							delete jQuerytempImg[0];
							jQuerytempImg = null;
							var angleOffset = 0;
						...
						jQuerytempImg.attr("src", this.result);
					} else {
		...
		jQueryfile.click(function() {
			this.value = "";
		});



		var jQuerycontainer, // 容器，包含裁剪视图层和遮罩层
			jQueryclipView, // 裁剪视图层，包含移动层
			jQuerymoveLayer, // 移动层，包含旋转层
			jQueryrotateLayer, // 旋转层
			jQueryview, // 最终截图后呈现的视图容器
			canvas, // 图片裁剪用到的画布
		...
		var jQueryok = jQuery(ok);
		if (jQueryok.length) {
			jQueryok.click(function() {
				clipImg();
			});
		}
		...
		function imgLoad() {
			...
			jQueryrotateLayer.append(this);

			hideAction.call(this, jQueryimg, function() {
				imgWidth = this.naturalWidth;
				imgHeight = this.naturalHeight;
			});

			hideAction(jQuerymoveLayer, function() {
				resetScroll();
			});
		...
		function initScroll() {
			...
			myScroll = new IScroll(jQueryclipView[0], options);
		}
		function resetScroll() {
			...
			jQueryrotateLayer.css({
				"width": imgWidth,
				"height": imgHeight
			});
			setTransform(jQueryrotateLayer, curX, curY, curAngle);
		...
		function refreshScroll(width, height) {
			...
			// iscroll 在刷新方法中正是使用了 offsetWidth/offsetHeight 来获取scroller元素(jQuerymoveLayer)的宽高
			// 因此需要手动将元素重新添加进文档，迫使浏览器强制更新元素的宽高
			jQueryclipView.append(jQuerymoveLayer);
			myScroll.refresh();
		}
		...
		function initEvent() {
			...
			if (is_mobile) {
				var hammerManager = new Hammer(jQuerymoveLayer[0]);
				...
				jQuerymoveLayer.on("dblclick", function(e) {
					rotateCW({
						x: e.clientX,
						y: e.clientY
					});
		...
		function rotateBy(angle, point) {
			...
			var loacl;
			if (!point) {
				loacl = loaclToLoacl(jQuerymoveLayer, jQueryclipView, clipWidth * .5, clipHeight * .5);
			} else {
				loacl = globalToLoacl(jQuerymoveLayer, point.x, point.y);
			}
			...
			setTransform(jQueryrotateLayer, curX, curY, curAngle, originX, originY);

			// 开始旋转
			setTransition(jQueryrotateLayer, curX, curY, newAngle, 200, function() {
				atRotation = false;
		...
		function clipImg() {
			...
			var local = loaclToLoacl(jQuerymoveLayer, jQueryclipView);
			...
			ctx.drawImage(jQueryimg[0], 0, 0);
			ctx.restore();

			var dataURL = canvas.toDataURL(outputType, 1);
			jQueryview.css("background-image", "url("+ dataURL +")");
			clipFinish.call(jQueryimg[0], dataURL);
		}


		function resize() {
			hideAction(jQuerycontainer, function() {
				containerWidth = jQuerycontainer.width();
				containerHeight = jQuerycontainer.height();
			});
		}
		function loaclToLoacl(jQuerylayerOne, jQuerylayerTwo, x, y) { // 计算jQuerylayerTwo上的x、y坐标在jQuerylayerOne上的坐标
			x = x || 0;
			y = y || 0;
			var layerOneOffset, layerTwoOffset;
			hideAction(jQuerylayerOne, function() {
				layerOneOffset = jQuerylayerOne.offset();
			});
			hideAction(jQuerylayerTwo, function() {
				layerTwoOffset = jQuerylayerTwo.offset();
			});
			return {
		...
		function globalToLoacl(jQuerylayer, x, y) { // 计算相对于窗口的x、y坐标在jQuerylayer上的坐标
			x = x || 0;
			y = y || 0;
			var layerOffset;
			hideAction(jQuerylayer, function() {
				layerOffset = jQuerylayer.offset();
			});
			return {
				x: x + jQuerywin.scrollLeft() - layerOffset.left,
				y: y + jQuerywin.scrollTop() - layerOffset.top
			};
		}
		function hideAction(jq, func) {
			var jQueryhide = jQuery();
			jQuery.each(jq, function(i, n){
				var jQueryn = jQuery(n);
				var jQueryhidden = jQueryn.parents().andSelf().filter(":hidden");
				var jQuerynone;
				for (var i = 0; i < jQueryhidden.length; i++) {
					if (!jQueryn.is(":hidden")) break;
					jQuerynone = jQueryhidden.eq(i);
					if (jQuerynone.css("display") == "none") jQueryhide = jQueryhide.add(jQuerynone.show());
				}
			});
		...
		function createImg(src) {
			...
			jQueryimg.load(imgLoad);
			jQueryimg.attr("src", src); // 设置图片base64值
		}

		function setTransform(jQueryobj, x, y, angle, originX, originY) {
			originX = originX || 0;
			originY = originY || 0;
			var style = {};
		...
		function setTransition(jQueryobj, x, y, angle, dur, fn) {
			// 这里需要先读取之前设置好的transform样式，强制浏览器将该样式值渲染到元素
			// 否则浏览器可能出于性能考虑，将暂缓样式渲染，等到之后所有样式设置完成后再统一渲染
			// 这样就会导致之前设置的位移也被应用到动画中
			jQueryobj.css(prefix + "transform");
			jQueryobj.css(prefix + "transition", prefix + "transform " + dur + "ms");
			jQueryobj.one(transitionEnd, function() {
				jQueryobj.css(prefix + "transition", "");
				fn.call(this);
			});
			jQueryobj.css(prefix + "transform", "translateZ(0) translate(" + x + "px," + y + "px) rotate(" + angle + "deg)");
		}

		function init() {
			// 初始化容器
			jQuerycontainer = jQuery(container).css({
				"user-select": "none",
				"overflow": "hidden"
			});
			if (jQuerycontainer.css("position") == "static") jQuerycontainer.css("position", "relative");

			// 创建裁剪视图层
			jQueryclipView = jQuery("<div class='photo-clip-view'>").css({
				"position": "absolute",
				"left": "50%",
				"top": "50%",
				"width": clipWidth,
			...
			jQuerymoveLayer = jQuery("<div class='photo-clip-moveLayer'>").appendTo(jQueryclipView);

			jQueryrotateLayer = jQuery("<div class='photo-clip-rotateLayer'>").appendTo(jQuerymoveLayer);

			// 创建遮罩
			var jQuerymask = jQuery("<div class='photo-clip-mask'>").css({
				"position": "absolute",
				"left": 0,
				"top": 0,
				"width": "100%",
			...
			var jQuerymask_left = jQuery("<div class='photo-clip-mask-left'>").css({
				"position": "absolute",
				"left": 0,
				"right": "50%",
				"top": "50%",
			...
			var jQuerymask_right = jQuery("<div class='photo-clip-mask-right'>").css({
				"position": "absolute",
				"left": "50%",
				"right": 0,
				"top": "50%",
			...
			var jQuerymask_top = jQuery("<div class='photo-clip-mask-top'>").css({
				"position": "absolute",
				"left": 0,
				"right": 0,
				"top": 0,
			...
			var jQuerymask_bottom = jQuery("<div class='photo-clip-mask-bottom'>").css({
				"position": "absolute",
				"left": 0,
				"right": 0,
				"top": "50%",
			...
			// 创建截取区域
			var jQueryclip_area = jQuery("<div class='photo-clip-area'>").css({
			...
			// 初始化视图容器
			jQueryview = jQuery(view);
			if (jQueryview.length) {
				jQueryview.css({
