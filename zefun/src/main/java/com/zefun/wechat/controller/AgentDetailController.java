...
//    private String openId = "oVeRPuAYeJBizrexSgVccJxd1tWg";
    ...
    public ModelAndView newStoreSelf(HttpServletRequest request, HttpServletResponse response) {
        ...
        return newStore(request, response, 3);
//        return newStore(request, response, 1);
    ...
    private ModelAndView newStore(HttpServletRequest request, HttpServletResponse response, int type) {
        ...
        if (type == 1) {
            ...
            view = View.AgentDetail.NEW_STORE_SELF;
            storeRecommendRecords = agentInfoService.getRecommendsByType(agentInfo.getAgentId(), 1);
        }
    ...
    public ModelAndView storeNormal(Integer page, String keyword, HttpServletRequest request, HttpServletResponse response) {
        ...
        ModelAndView mav = new ModelAndView(View.AgentDetail.STORE_NORMAL);
        ...
        if (storeRecommendRecords != null && !storeRecommendRecords.isEmpty()) {
            ...
            List<StoreAccount> storeAccounts = storeInfoService.getOrderedAccountByStoreIds(storeIds);
            totalStore = storeAccounts.size();
            totalWechat = storeInfoService.countWechatByIds(storeIds);
            ...
            if (storeAccounts != null && !storeAccounts.isEmpty()) {
                ...
                storeInfos = storeInfoService.getOrderedStore(storeIds, keyword, page, 10);
        ...
        mav.addObject("totalWechat", totalWechat);
        return mav;
...
//        return oldStore(request, response, 1); // 正常使用
    ...
    private ModelAndView oldStore(HttpServletRequest request, HttpServletResponse response, int type) {
        ...
        if (storeRecommendRecords != null && !storeRecommendRecords.isEmpty()) {
            ...
            //过滤的storeAccounts才是符合要求的门店账户信息(因为要根据type执行不同查询)
    ...
    public ModelAndView stat(String cityName, Integer orderType, HttpServletRequest request, HttpServletResponse response) {
        ...
        List<RegionCountRankDto> rankDtos = storeInfoService.statStoreRank(cityName, orderType);
        ModelAndView mav = new ModelAndView(View.AgentDetail.STORE_REGION_RANK);
        mav.addObject("cityName", cityName);
        mav.addObject("orderType", orderType);
        mav.addObject("rankDtos", rankDtos);
        return mav;
    }
