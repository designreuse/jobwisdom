...
public class WeixinUploadService {
    ...
    * @return 返回状态
    ...
    public BaseDto uploadNews(String ACCESS_TOKEN, ThumbDto thumb,
        ...
        if (status > 0){
            return new BaseDto(App.System.API_RESULT_CODE_FOR_SUCCEES, mediaId);
//            return new BaseDto(App.System.API_RESULT_CODE_FOR_SUCCEES, JSONArray.fromObject(automaticReplies));
    ...
    * @return 跳转页面
    ...
    public BaseDto updateItems(UpdateItemsDto updateItemsDto, UpdateArticleDto updateArticleDto, 
        ...
        if (ok == 0){
            ...
            return new BaseDto(App.System.API_RESULT_CODE_FOR_FAIL, WeixinConfigService.getErrorMessage().get(ok+""));
        }
    }
    ...
    * @return 返回页面
    ...
    public ModelAndView aritcManager(Integer storeId) {
        String storeName = storeInfoMapper.selectBaseInfoByStoreId(storeId).getStoreName();
    ...
    * @return 状态码
     ...
     * 图文预览
    * @author 高国藩
    * @date 2015年12月21日 下午7:54:12
    * @param accessToken                微信忍者龟
    * @param userName                   账户名
    * @param mediaId                    媒体ID
    * @param type                       类型
    * @return                           是否推送成功
    ...
    * @throws IOException               异常
    ...
    public BaseDto prevItems(String accessToken, String userName,
        ...
        if (touser == null){
            ...
            StringBuffer sql = new StringBuffer();
            sql.append("{'touser':"+"'"+touser+"'"+",");
            sql.append("'mpnews':{'media_id':"+"'"+mediaId+"'"+"},");
            sql.append("'msgtype':'mpnews'}");
            StringEntity stringEntity =  new StringEntity(JSONObject.fromObject(sql.toString()).toString(), "UTF-8");
            CloseableHttpClient httpClient = HttpClientBuilder.create().build();
            HttpPost httpPost = new HttpPost(Url.Wechat.ITEM_PRE_VIEW.replace("ACCESS_TOKEN", accessToken));
            httpPost.setEntity(stringEntity);
            HttpResponse httpResponse = httpClient.execute(httpPost);
            HttpEntity entity = httpResponse.getEntity();
            JSONObject response = JSONObject.fromObject(EntityUtils.toString(entity, "utf-8"));
            if (response.get("errcode").toString().equals("0")){
                return new BaseDto(App.System.API_RESULT_CODE_FOR_SUCCEES, "预览信息已发送至您的手机");
            }
            else {
                return new BaseDto(App.System.API_RESULT_CODE_FOR_FAIL, WeixinConfigService.getErrorMessage().get(response.getString("errcode")));
            }
        }
    }
