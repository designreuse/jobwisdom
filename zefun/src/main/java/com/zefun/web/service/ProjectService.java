...
public class ProjectService {
    ...
    * @return   所有部门下的轮牌信息
    ...
    public List<DeptMahjongDto> getDeptMahjongByStoreId(int storeId) {
        ...
        List<DeptMahjongDto> deptMahjongList = new ArrayList<DeptMahjongDto>(
                deptIdList.size());
        for (int i = 0; i < deptIdList.size(); i++) {
            DeptMahjongDto deptMahjong = getDeptMahjongByDeptId(
                    deptIdList.get(i));
    ...
    * @return       轮牌信息
    ...
    public DeptMahjongDto getDeptMahjongByDeptId(int deptId) {
        String deptProjectBaseInfoJson = redisService
                .hget(App.Redis.DEPT_PROJECT_MAHJONG_INFO_KEY_HASH, deptId);
        ...
        if (StringUtils.isBlank(deptProjectBaseInfoJson)) {
            ...
            for (int i = 0; i < deptMahjong.getMahjongLevelList().size(); i++) {
                String positionIds = deptMahjong.getMahjongLevelList().get(i)
                        .getPositionId();
                ...
                for (int j = 0; j < positionIds.split(",").length; j++) {
                    list.add(Integer
                            .parseInt(positionIds.split(",")[j].split(":")[0]));
                }
                Map<String, String> param = new HashMap<>();
                param.put("positionIds", JSONArray.fromObject(list).toString()
                        .replace("[", "").replace("]", "").trim());
                List<EmployeeLevelBaseDto> baseDtos = employeeLevelMapper
                        .selectByPositionIds(param);
                deptMahjong.getMahjongLevelList().get(i)
                        .setEmployeeLevelList(baseDtos);
            }
            // if (deptMahjong == null) {
            // return null;
            // }
            redisService.hset(App.Redis.DEPT_PROJECT_MAHJONG_INFO_KEY_HASH,
                    deptId, EntityJsonConverter.entity2Json(deptMahjong));
        }
        // 缓存中存在则直接转换为对象
        else {
            deptMahjong = EntityJsonConverter
                    .json2Entity(deptProjectBaseInfoJson, DeptMahjongDto.class);
        }
        return deptMahjong;
    }
    ...
    * @return List<Integer>
     */
    public List<Integer> queryShiftMahjongByLevelId(
            List<Integer> shiftMahjongIds) {
        return shiftMahjongEmployeeMapper
                .selectShiftMahjongByLevelId(shiftMahjongIds);
    }
    ...
    * @return int
    ...
    public int deleteProjectCategory(Integer categoryId) {
        ...
        return projectCategoryMapper
                .updateByPrimaryKeySelective(projectCategory);
    }
    ...
    * @return List<ProjectCategory>
     */
    public List<ProjectCategory> queryProjectCategoryList(
    ...
    * @return List<ProjectDiscount>
     */
    public List<ProjectCommission> queryProjectCommissionList(
    ...
    * @return List<ProjectDiscount>
     */
    public List<ProjectDiscount> queryProjectDiscountList(
    ...
    * @return Integer
     */
    @Transactional
    public Integer saveProject(Integer userId, ProjectInfo projectInfo,
            String[] levelId, String[] discountProportion,
            String[] discountAmount, String[] onlineAppointmentPrice,
            String[] empLevelId, String[] assignType, String[] assignCash,
            String[] assignCard, String[] nonAssignCash, String[] nonAssignCard,
            String[] appointmentReward, String[] shiftMahjongId,
            String[] shiftStepNameArr, String[] stepPerformanceArr,
            String[] isDisableArr, String[] zhiweinum,
            String[] stepPerformanceType, String[] stepPerformance) {
        /*
         * List<StoreUser> storeUserList =
         * storeInfoService.queryStoreUserList(userId);
         ...
         * saveCommissionAndDiscount(userId, totalStoreProjectId, levelId,
         * discountProportion, discountAmount, onlineAppointmentPrice,
         * empLevelId,
         * assignType, assignCash, assignCard, nonAssignCash, nonAssignCard,
         * appointmentReward,
         ...
         * saveCommissionAndDiscount(userId, projectId, levelId,
         * discountProportion, discountAmount, onlineAppointmentPrice,
         * empLevelId,
         * assignType, assignCash, assignCard, nonAssignCash, nonAssignCard,
         * appointmentReward,
        ...
        // Integer projectId = projectInfo.getProjectId();
        // saveCommissionAndDiscount(userId, projectId, levelId,
        // discountProportion, discountAmount,
        // onlineAppointmentPrice, empLevelId, assignType,
        // assignCash, assignCard, nonAssignCash, nonAssignCard,
        // appointmentReward, shiftMahjongId, shiftStepNameArr,
        // stepPerformanceArr,
        // isDisableArr, zhiweinum);
        // return projectId;
        //// }
        // String[] shiftStepNameArr, String[] stepPerformanceArr, String[]
        // isDisableArr) {
        // List<StoreUser> storeUserList =
        // storeInfoService.queryStoreUserList(userId);
        // if (storeUserList.size() > 1) {
        ...
        saveCommissionAndDiscount(userId, projectId, levelId,
                ...
                nonAssignCard, appointmentReward, shiftMahjongId,
                shiftStepNameArr, stepPerformanceArr, isDisableArr, zhiweinum,
                stepPerformanceType, stepPerformance);
        return projectId;
        // }
    }
    ...
    * @return int
     */
    @Transactional
    public int updateProject(Integer userId, ProjectInfo projectInfo,
            String[] levelId, String[] discountProportion,
            String[] discountAmount, String[] onlineAppointmentPrice,
            String[] empLevelId, String[] assignType, String[] assignCash,
            String[] assignCard, String[] nonAssignCash, String[] nonAssignCard,
        ...
        // 2.再新增
        saveCommissionAndDiscount(userId, projectInfo.getProjectId(), levelId,
                discountProportion, discountAmount, onlineAppointmentPrice,
                empLevelId, assignType, assignCash, assignCard, nonAssignCash,
                nonAssignCard, appointmentReward, shiftMahjongId,
                shiftStepNameArr, stepPerformanceArr, isDisableArr, zhiweinum,
        ...
        List<ProjectInfo> queryProjectInfoList = projectInfoMapper
                .selectByProperty(queryProjectInfo);
        for (int i = 0; i < queryProjectInfoList.size(); i++) {
            ...
            // 2.再新增
            saveCommissionAndDiscount(userId, resultProjectId, levelId,
                    discountProportion, discountAmount, onlineAppointmentPrice,
                    empLevelId, assignType, assignCash, assignCard,
                    nonAssignCash, nonAssignCard, appointmentReward,
                    shiftMahjongId, shiftStepNameArr, stepPerformanceArr,
         ...
         * deleteCommissionAndDiscount(projectInfo.getProjectId());
         * // 2.再新增
         * saveCommissionAndDiscount(userId, projectInfo.getProjectId(),
         * levelId, discountProportion, discountAmount, onlineAppointmentPrice,
         * empLevelId, assignType, assignCash, assignCard, nonAssignCash,
         * nonAssignCard, appointmentReward, shiftMahjongId, shiftStepNameArr,
         * isDisableArr);
         * // 分店项目修改
         * ProjectInfo queryProjectInfo = new ProjectInfo();
         * queryProjectInfo.setParentId(projectInfo.getProjectId());
         * List<ProjectInfo> queryProjectInfoList =
         * projectInfoMapper.selectByProperty(queryProjectInfo);
         * for (int i = 0; i < queryProjectInfoList.size(); i++) {
         * ProjectInfo resultProjectInfo = queryProjectInfoList.get(i);
         * Integer resultProjectId = resultProjectInfo.getProjectId();
         * projectInfo.setProjectId(resultProjectId);
    ...
    * @param stepPerformance 业绩值
     */
    private void saveCommissionAndDiscount(Integer userId, Integer projectId,
            String[] levelId, String[] discountProportion,
            String[] discountAmount, String[] onlineAppointmentPrice,
            String[] empLevelId, String[] assignType, String[] assignCash,
            String[] assignCard, String[] nonAssignCash, String[] nonAssignCard,
        ...
        if (levelId != null && levelId.length > 0) {
            ...
            for (int i = 0; i < levelId.length; i++) {
                ...
                // 页面上已经去掉了这个了
                projectDiscount.setDiscountProportion(
                        new BigDecimal(discountAmount[i].trim()));
                projectDiscount.setDiscountAmount(
                        new BigDecimal(discountAmount[i].trim()));
                projectDiscount.setOnlineAppointmentPrice(
                        new BigDecimal(discountAmount[i].trim()));
            ...
            projectDiscountMapper
                    .insertProjectDiscountList(projectDiscountList);
        }
        ...
        for (int i = 0; i < zhiweinum.length; i++) {
            for (int j = 0; j < Integer.parseInt(zhiweinum[i]); j++) {
                ...
                projectCommission
                        .setLevelId(Integer.parseInt(empLevelId[num].trim()));

                projectCommission.setAssignCashType(
                        Integer.parseInt(assignType[num].trim()));
        ...
        projectCommissionMapper
                .insertProjectCommissionList(projectCommissionList);
        ...
        for (int i = 0; i < shiftMahjongId.length; i++) {
            ...
            projectStep.setStepPerformanceType(
                    Integer.parseInt(stepPerformanceType[i]));
    ...
    * @param projectId 项目id
    ...
    public void deleteCommissionAndDiscount(Integer projectId) {
        ...
        ProjectDiscount projectDiscount = new ProjectDiscount();
        projectDiscount.setProjectId(projectId);
        List<ProjectDiscount> projectDiscountList = projectDiscountMapper
                .selectByProperty(projectDiscount);
        for (int i = 0; i < projectDiscountList.size(); i++) {
            projectDiscountMapper.deleteByPrimaryKey(
                    projectDiscountList.get(i).getDiscountId());
        }
    }
    ...
    * @param projectId 项目id
    ...
    public void deleteProject(Integer projectId) {
        ...
        List<ProjectInfo> resultProjectInfoList = projectInfoMapper
                .selectByProperty(queryProjectInfo);
        if (resultProjectInfoList.size() > 0) {
            for (int i = 0; i < resultProjectInfoList.size(); i++) {
                ...
                projectInfoMapper
                        .updateByPrimaryKeySelective(resultProjectInfo);
    ...
    * @param projectId 项目id
    ...
    private void updateCommissionAndDiscount(Integer projectId) {
        ...
        ProjectCommission projectCommission = new ProjectCommission();
        projectCommission.setProjectId(projectId);
        List<ProjectCommission> projectCommissionList = projectCommissionMapper
                .selectByProperty(projectCommission);
    ...
    * @return   会员实际价格
     */
    public BigDecimal getProjectPriceByMember(int levelId, int projectId,
            BigDecimal projectPrice, Integer storeId) {
        // 计算会员折扣价
        BigDecimal discountAmount = projectPrice;
        Map<String, Integer> map = new HashMap<String, Integer>();
        map.put("levelId", levelId);
        map.put("projectId", projectId);
        ProjectDiscount discount = projectDiscountMapper
                .selectDiscountPorjectIdAndLevelId(map);
        // 如果没有特定会员价，那么计算查找该会员的折扣去计算
        if (discount == null) {
            Map<String, Integer> levelMap = new HashMap<>();
            levelMap.put("levelId", levelId);
            levelMap.put("storeId", storeId);
            MemberLevelDiscount memberLevelDiscount = memberLevelDiscountMapper
                    .selectByStoreLevel(levelMap);
            if (memberLevelDiscount != null) {
                discountAmount = discountAmount
                        .multiply(new BigDecimal(
                                memberLevelDiscount.getProjectDiscount()))
    ...
    * @return List<ProjectInfoDto>
     */
    public List<ProjectInfoDto> queryProjectInfoList(
    ...
    * @return List<ProjectCommission>
     */
    public List<ProjectCommissionDto> queryProjectCommissionByProjectId(
            Integer projectId) {
    ...
    * @return BaseDto
    ...
    public BaseDto saveProjectInfo(ProjectInfo projectInfo, Integer stepNum) {
        ...
        redisService.hdel(App.Redis.DEPT_PROJECT_BASE_INFO_KEY_HASH,
    ...
    * @return BaseDto
     */
    public BaseDto updateProjectInfoPrice(ProjectInfo projectInfo,
            Integer stepNum) {
        projectInfo.setProjectStep(stepNum);
        projectInfoMapper.updateByPrimaryKeySelective(projectInfo);
        JSONObject data = new JSONObject();
    ...
    * @return xiu
    ...
    public BaseDto saveOrUpdateCommison(JSONObject data, Integer status) {
        ...
        List<ProjectStep> oldProjectSteps = projectStepMapper
                .queryProjectStepByPIdList(data.getInt("projectId"));
        ...
        List<ProjectCommission> projectCommissions = projectCommissionMapper
                .selectByProperty(projectCommission);
        for (int i = 0; i < projectCommissions.size(); i++) {
            projectCommissions.get(i).setIsDeleted(1);
            projectCommissionMapper
                    .updateByPrimaryKeySelective(projectCommissions.get(i));
        }

        List<ProjectStep> projectSteps = (List<ProjectStep>) JSONArray
                .toCollection(data.getJSONArray("step"), ProjectStep.class);
        for (int i = 0; i < projectSteps.size(); i++) {
            projectStepMapper.insert(projectSteps.get(i));
        }
        List<ProjectCommission> commissions = (List<ProjectCommission>) JSONArray
                .toCollection(data.getJSONArray("commission"),
        ...
        ProjectInfo projectInfo = projectInfoMapper
                .selectByPrimaryKey(data.getInt("projectId"));
        if (status == 0) {
    ...
    * @return     BaseDto
    ...
    public BaseDto saveLevelDiscount(JSONObject data) {
        ...
        ProjectDiscount projectDiscount = new ProjectDiscount();
        projectDiscount.setProjectId(projectId);
        List<ProjectDiscount> discounts = projectDiscountMapper
                .selectByProperty(projectDiscount);
        ...
        discounts = (List<ProjectDiscount>) JSONArray
                .toCollection(data.getJSONArray("data"), ProjectDiscount.class);
    ...
    * @return BaseDto
    ...
    public BaseDto updateProjectBystepNum(JSONObject data, Integer stepNum) {
        if (stepNum == 1) {
            ProjectInfo projectInfo = (ProjectInfo) JSONObject.toBean(data,
                    ProjectInfo.class);
            projectInfoMapper.updateByPrimaryKeySelective(projectInfo);
            JSONObject returnDate = new JSONObject();
            returnDate.put("projectId", projectInfo.getProjectId());
    ...
    public ModelAndView projectCategoryView(String storeAccount, Integer storeId, Integer deptId, HttpServletRequest request) {
        ...
        if (storeInfos.size() > 0){
            ...
            if (projectCategories.size() > 0 && goodsCategories.size() > 0){
                projectCategories = projectCategories.stream().filter(p -> p.getDeptId().equals(deptIds)).collect(Collectors.toList());
                goodsCategories = goodsCategories.stream().filter(g -> g.getDeptId().equals(deptIds)).collect(Collectors.toList());
            }
    ...
    * @param projectStep               岗位信息
    * @param ProjectCommission         提成信息
    * @return                          新增状态(返回项目信息)
     */
    ...
    public BaseDto saveProjectOrUpdate(Integer storeId, ProjectInfo projectInfo,
            List<ProjectDiscount> discounts, List<ProjectStep> projectStep,
            List<ProjectCommission> ProjectCommission) {

        if (projectInfo.getProjectId() != null) {
            projectInfoMapper.updateByPrimaryKeySelective(projectInfo);
            projectStepMapper.deleteByProjectId(projectInfo.getProjectId());
            projectCommissionMapper.deleteByProjectId(projectInfo.getProjectId());
        } 
        else {
            projectInfoMapper.insertSelective(projectInfo);
        }
        ...
        List<ProjectDiscount> hasDiscounts = projectDiscountMapper
                .selectByProperty(projectDiscount);
        ...
        if (ProjectCommission.size() != 0) {
            projectCommissionMapper.insertTiveList(ProjectCommission);
        }

    
        for (int i = 0; i < projectStep.size(); i++) {
            BigDecimal stepPerformance = projectStep.get(i)
                    .getStepPerformance();
            if (stepPerformance != null) {
                projectStep.get(i).setIsDeleted(0);
                projectStep.get(i).setProjectId(projectInfo.getProjectId());;
                projectStepMapper.insertStep(projectStep.get(i));
            }
        }
        redisService.hdel(App.Redis.DEPT_PROJECT_BASE_INFO_KEY_HASH,
                projectInfo.getDeptId());
