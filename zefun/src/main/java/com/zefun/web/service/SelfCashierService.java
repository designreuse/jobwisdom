...
//    private Logger logger = Logger.getLogger(SelfCashierService.class);
	...
	* @param memberId	   结账会员标识(会员自助结账时)
	* @return  成功返回码0；失败返回其他错误码，返回值为提示语
	 * @throws ServiceException 
	 */
	@Transactional
	public BaseDto cashierSubmit(int employeeId, OrderInfoSubmitDto orderSubmit, Integer memberId) throws ServiceException {
	    ...
	    for (OrderDetaiSubmitDto detail : orderSubmit.getDetailList()) {
            ...
            if (ownerMemberId != null) {
                ...
                //优惠类型为礼金
                    ...
                    //增加礼金流水
                    insertGiftMoneyFlow(ownerMemberId, ownerDetail.getDetailId(), detail.getOffAmount(), curTime, ownerDetail.getProjectName());
                    
                    giftAmount = giftAmount.add(detail.getOffAmount());
                    orderDetail.setGiftAmount(detail.getOffAmount());
                    
                    //检查是否有礼金抵扣
                    if (StringUtils.isNotBlank(ownerDetail.getFreeAmount()) 
                            && !"0".equals(ownerDetail.getFreeAmount()) && !"0.00".equals(ownerDetail.getFreeAmount())) {
	    ...
	    BigDecimal tempAmount = cashierDto.getDebtAmount().add(orderSubmit.getAlipayAmount()).
	            ...
	            add(appointOff).add(comboAmount).add(couponAmount).add(orderSubmit.getGroupAmount()).
	            add(orderSubmit.getDebtAmount());
	    ...
	    if (orderSubmit.getCardAmount().compareTo(BigDecimal.ZERO) == 1) {
	        ...
	        //新增会员账户流水
	        insertMoneyFlow(cashierDto.getMemberInfo().getStoreId(), orderSubmit.getSubAccountId(), orderId, orderSubmit.getCardAmount(), 
	                cashierDto.getMemberInfo().getBalanceAmount(), curTime, employeeId);
	        //新增积分流水
	        if (integralAmount > 0) {
	            insertIntegralFlow(ownerMemberId, orderId, cashierDto.getMemberInfo().getBalanceIntegral(), integralAmount, curTime);
	        }
	    }
	    ...
	    if (ownerMemberId != null) {
		    ...
		    if (orderSubmit.getDebtAmount().compareTo(BigDecimal.ZERO) == 1) {
		    	//增加挂帐记录
		    	memberInfoService.insertDebtFlow(ownerMemberId, orderId, orderSubmit.getDebtAmount(), "消费欠款", 1, employeeId, curTime);
	    ...
	    if (orderSubmit.getIsNotify() == 1 && employeeId != 0 && cashierDto.getOrderStatus() != 5) {
	        ...
	        if (StringUtils.isNotBlank(openId)) {
	            ...
	            String url = App.System.SERVER_BASE_URL + Url.MemberCenter.VIEW_PAYMENT_DETAIL.replace("{businessType}", "1") + "?orderId=" + orderId;
	            rabbitService.sendPaymentNotice(2, cashierDto.getMemberInfo().getStoreId(), url, openId, memberInfo.getStoreName(), 
	                    memberInfo.getPhone(), projectList, cashierDto.getReceivableAmount().toString(), orderSubmit.getCardAmount().toString(), 
	                    memberInfo.getBalanceAmount().subtract(orderSubmit.getDebtAmount()).toString(), orderSubmit.getDebtAmount().toString());
	        }
	    }
    ...
    * @param recommenderId	分享者
    * @param orderId	首次消费的订单标识
    * @param recommendedInfo	被推荐者信息
     */
	    	...
	    	for (MemberSubAccountDto subAccount : subAccountList) {
	    		...
	    		for (SelfCashierDetailDto detail : orderInfo.getOrderDetails()) {
	    			...
	    			if (detail.getOrderType() == 1) {
	    				price = projectService.getProjectPriceByMember(levelId, detail.getProjectId(), detail.getProjectPrice());
	    			}
	    			else if (detail.getOrderType() == 2) {
	    				price = goodsInfoService.getGoodsPriceByMember(levelId, detail.getProjectId(), detail.getProjectPrice());
	    			}
	    			else {
