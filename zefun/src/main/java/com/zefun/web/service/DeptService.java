    ...
    public BaseDto importExcle(MultipartFile file, String temp, Integer storeId) throws IOException{
            ...
            if (row.getRowNum() > 0){
                ...
                while (cells.hasNext()) {
                    ...
                    if (cell.getColumnIndex() == 0){
                        ...
                        if (returnValue!=null &&returnValue!=""){
                            ...
                            copyorg.setIsDept(Integer.parseInt(returnValue));
                        }
                    }
                ...
                while (cellss.hasNext()) {
                    ...
                    //TODO
                    	...
                    	String returnValue = changeCellToString(cell);
                        if (returnValue==null||returnValue==""){
                            return new BaseDto(-1, "第 "+(row.getRowNum()+1)+"行是否跨部门不能为空！");
                        }
                        if (returnValue.equals("是")){
                            returnValue="1";
                        }
                            ...
                            return new BaseDto(-1, "第 "+(row.getRowNum()+1)+"行是否跨部门只能填写是或否！");
                        }
                        org.setIsDept(Integer.parseInt(returnValue));
                    }
                    else if (cell.getColumnIndex() == 6){
                        String returnValue = changeCellToString(cell);
                        if (org.getPositionCode()==null){
        ...
        for (int i = 0; i < listDto.size(); i++) {
            ...
            if (info==null){
                ...
                deptInfoMapper.insert(deptInfo);
                /**
                 * 如果是该部门需要新增,也需要初始化该部门班次信息(shift_into)
                 */
                //查看该店铺该部门没有班次信息,初始化班次
                if (shiftMapper.selectCountShiftByStoreAndDept(storeId, deptInfo.getDeptId()) == 0) {
                	addshift(deptInfo.getStoreId(), deptInfo.getDeptId());
                }
            ...
            if (org.getPositionCode()!=null){
                ...
                positioninfo.setIsDept(org.getIsDept());
                ...
                if (org.getLevelName()!=null&&org.getLevelName()!=""){
                    ...
                    List<EmployeeLevel> info2=employeelevelMapper.islevelName(EntityJsonConverter.
                    ...
                    if (info2.size()==0){
                        ...
                        employeelevelMapper.insert(EntityJsonConverter.json2Entity(JSONObject.fromObject(map).toString(), EmployeeLevel.class));
                    }
