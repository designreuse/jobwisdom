...
<div class="head text-center ">
  ...
  <c:forEach items="${dtoList}" var="dto" varStatus="status">
      <div <c:if test="${dto.deptId == employeeDeptId}">class="bd-white bodyoh show"</c:if> <c:if test="${dto.deptId != employeeDeptId}">class="bd-white bodyoh hide"</c:if> id = "${dto.deptId}">
                                    ...
                                    <c:forEach items="${projectInfoListOrder}" var="projectInfoOrder" varStatus="status">
                                       <li name = "${projectInfoOrder.projectId}" types = "1">
                                            ...
                                            <div class="item-desc" name="${projectInfoOrder.projectImage}">
                                                ...
                                                <div class="origin-price">已服务:${projectInfoOrder.salesCount}</div>
                                            </div>
...
<script type="text/javascript">
    ...
    $(function () {
    	 //如果当前没有人员所属部门没有项目信息，自动切换到第一个存在项目的部门
    	 if($(".bd-white.bodyoh.show").length == 0){$(".bd-white.bodyoh").eq(0).removeClass('hide')}
         ...
         jQuery("*").click(function (event) {
             ...
             if (jQuery(state).hasClass("bumen")) {
                 jQuery("#yg-select-modal").removeClass("hide");
             }
             else {
                 jQuery("#yg-select-modal").addClass("hide");
             }
        ...
        showMemberInfo('${memberBaseDto.memberId }');
    })
    ...
    $("#choiceEmployee").click(function (){
        ...
        mpt.name = "memberId";
        mpt.value = "${memberBaseDto.memberId}";
        
        var sex = document.createElement("textarea");
        sex.name = "sex";
        sex.value = "${memberBaseDto.sex}";
