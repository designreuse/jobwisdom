...
<div class="s-modal-miss no-padding s-gouwuche-modal hide" id="yg-state">
    <div class="yg-state">
      <div class="yg-state-in">
      ...
      <!--员工位置-->
      <div class="yg-weizhi">
            <div class="yg-weizhi-in">
                <ul>
                    <li onclick="toTopIMG()"><span class="iconfont icon-yidaozuiqian"></span><p>移到最前</p></li>
                    <li onclick="upwardIMG()"><span class="iconfont icon-qianjinyibu"></span><p class="wz-current">移前一位</p></li>
                    <li onclick="nextIMG()"><span class="iconfont icon-houtuiyibu"></span><p>移后一位</p></li>
                    <li onclick="toEndIMG()"><span class="iconfont icon-yidaozuihou"></span><p>移动最后</p></li>
...
<script>
		...
		$.ajax({
			...
			success : function(e){
				...
				for (var i = 0; i < shiftMahjongDtoList.length; i++) {
					...
					var swiperWrapper = $("<div class='swiper-wrapper' nature = '"+shiftInfo.nature+"'></div>");
	...
	function infoDIV(shiftMahjongEmployeeList, swiperWrapper){
		...
		for (var j = 0; j < shiftMahjongEmployeeList.length; j++) {
			...
			var slideObj = jQuery("<div class='swiper-slide score-shop-li line-height-center' shiftMahjongEmployeeId = '"+shiftMahjongEmployee.shiftMahjongEmployeeId+"' state = '"+shiftMahjongEmployee.state+"' shiftMahjongId = '"+shiftMahjongEmployee.shiftMahjongId+"' shiftMahjongOrder = '"+shiftMahjongEmployee.shiftMahjongOrder+"' onclick = 'openModel(this)'></div>");
	...
	function openModel(obj){
		...
		shiftMahjongEmployeeId = $(obj).attr("shiftMahjongEmployeeId");
	    shiftMahjongId = $(obj).attr("shiftMahjongId");
	    ...
	    jQuery.ajax({
			...
			success : function(e){
                ...
                infoDIV(date, overallObj);
			}
		});
	    
		var nature = $(overallObj).attr("nature");
		
		var states = $(overallObj).find("div[shiftMahjongEmployeeId='"+shiftMahjongEmployeeId+"']").attr("state");
		
		var shiftMahjongOrders = $(overallObj).find("div[shiftMahjongEmployeeId='"+shiftMahjongEmployeeId+"']").attr("shiftMahjongOrder");
		
		if (states == 0 || states == 4) {
			$("#yg-state").find(".yg-state-in").addClass("hide");
			$("#yg-state").find(".yg-weizhi").removeClass("hide");
		}
		else if ((nature == 1 && states == 2) || states == 1) {
			$("#yg-state").find(".yg-state-in").removeClass("hide");
			$("#yg-state").find(".yg-weizhi").removeClass("hide");
		}
		else {
			$("#yg-state").find(".yg-state-in").removeClass("hide");
			$("#yg-state").find(".yg-weizhi").addClass("hide");
		}
		
		oldState = states;

		shiftMahjongOrder = shiftMahjongOrders;
	...
	function upwardIMG(){
		jQuery.ajax({
			type : "post",
			url : baseUrl + "staff/action/upwardIMG",
			data : "shiftMahjongEmployeeId="+shiftMahjongEmployeeId+"&shiftMahjongId="+shiftMahjongId+"&shiftMahjongOrder="+shiftMahjongOrder,
			async:false,//使用同步的Ajax请求 
			dataType : "json",
			success : function(e){
				if(e.code != 0){
	...
	function nextIMG(){
		jQuery.ajax({
			type : "post",
			url : baseUrl + "staff/action/nextIMG",
			data : "shiftMahjongEmployeeId="+shiftMahjongEmployeeId+"&shiftMahjongId="+shiftMahjongId+"&shiftMahjongOrder="+shiftMahjongOrder,
			async:false,//使用同步的Ajax请求 
			dataType : "json",
			success : function(e){
	...
	//置顶
	function toTopIMG(){
		if (shiftMahjongOrder == 1) {
			return;
		}
		jQuery.ajax({
			type : "post",
			url : baseUrl + "staff/action/toTopIMG",
			data : "shiftMahjongEmployeeId="+shiftMahjongEmployeeId+"&shiftMahjongId="+shiftMahjongId+"&shiftMahjongOrder="+shiftMahjongOrder,
			async:false,//使用同步的Ajax请求 
			dataType : "json",
			success : function(e){
	...
	function toEndIMG(){
		...
		jQuery.ajax({
			type : "post",
			url : baseUrl + "staff/action/toEndIMG",
			data : "shiftMahjongEmployeeId="+shiftMahjongEmployeeId+"&shiftMahjongId="+shiftMahjongId+"&shiftMahjongOrder="+shiftMahjongOrder,
			async:false,//使用同步的Ajax请求 
			dataType : "json",
			success : function(e){
