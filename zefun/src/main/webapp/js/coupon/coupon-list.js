	...
	jQuery("#baocun").on("click",function(){
		if(jQuery("#couponStopTime").val().length>10){
			dialog("请注意时间格式");
			return;
		}
		if(ctype=="0"){
	...
	function changeEniy(type){
		...
		if(type=="4"){
			jQuery("#select_view").empty();
			var select = jQuery("<select id='meal' data-placeholder='请选择...' class='input-medium hide chzn-select'></select>");
			select.append(jQuery("<option value='0'>请选择项目系列</option>"));
			for (var i = 0; i < projectCategories.length; i++) {
				select.append(jQuery("<option value='"+projectCategories[i].categoryId+"'>"+projectCategories[i].categoryName+"</option>"));
			}
			jQuery("#select_view").append(select);
			select.chosen();
		}
		if(type=="5"){
			jQuery("#select_view").empty();
			var select = jQuery("<select id='meal' data-placeholder='请选择...' class='input-medium hide chzn-select'></select>");
			select.append(jQuery("<option value='0'>请选择商品系列</option>"));
			for (var i = 0; i < goodsCategories.length; i++) {
				select.append(jQuery("<option value='"+goodsCategories[i].categoryId+"'>"+goodsCategories[i].categoryName+"</option>"));
			}
			jQuery("#select_view").append(select);
			select.chosen();
		}
		...
		jQuery.ajax({
			...
			success : function(e){
				if(e.code == 0){
					...
					clickDoc.parent().children(".fs").remove();
				}
			}
		});
	});
       ...
       jQuery(':input','#couponsForm') 
	    ...
	    jQuery("#select_view").empty();
	   	...
	   	var str = '<option selected="selected" value="0">通用</option><option value="1">项目</option><option value="2">商品</option>'
	   		+ '<option value="4">项目系列</option><option value="5">商品系列</option>';
