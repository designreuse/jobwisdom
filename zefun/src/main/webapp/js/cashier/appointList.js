...
jQuery("div[name = 'memberTR']").delegate("span[name='breakName']","click", function(event){
    ...
    //在查询好了会员信息后，再点击"放大镜"，需要把姓名框框显示出来
    jQuery("#customerNameSpan").show();
...
function submitPhone (obj) {
	...
	jQuery.ajax({
		...
		success : function(e){
			...
			/*
			 * 该方法是点击"放大镜"查询会员
			 * memberBaseDto.memberId有值就隐藏客户姓名(id="customerName")框框，代表该客户是会员不需要输入姓名
			 * memberBaseDto.memberId无值就显示客户姓名框框(在另外地方)
			 */
			jQuery(obj).parents("div[name='memberTR']").find("input[name='memberId']").val(memberBaseDto.memberId).change();
			if (memberBaseDto.memberId != null || memberBaseDto.memberId.length != 0) {
				jQuery("#customerNameSpan").hide();
				jQuery("#customerName").hide();
				globalCustomerName = memberBaseDto.name;
				globalCustomerPhone = memberBaseDto.phone;
			}
		    ...
		    jQuery(obj).parents(".tab-form").find("span[name='needRefund']").text(zeroValue(memberBaseDto.debtAmount));
		}
	});
}
