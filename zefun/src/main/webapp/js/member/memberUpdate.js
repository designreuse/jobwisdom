...
jQuery("body").delegate(".update","click",function(){
	...
	jQuery("input[name='community']").val(community);
...
/**修改备注信息*/
function changeCommunity(obj){
	jQuery("input[name='community']").val(jQuery(obj).val());
}
/** 提交 */
	...
	jQuery.ajax({
		...
		success : function(e){
			if(e.code == 0){
				...
				var community = jQuery("input[name='community']").val();
