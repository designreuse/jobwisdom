...
function sortByDept(deptId,dateType,deptName){
	jQuery.ajax({
		type:"post",
		data:{"deptId":deptId,"dateType":dateType,"deptName":deptName},
		url:baseUrl + "laborperformance/view/laborperformance/GoodsRankByDate",
		dataType:"json",
...
function refreshData(projectList){
	var projectRankList=projectList;
	var tbody=document.createElement("tbody");
	if(projectRankList.length==0){
		dialog("本部门当前没有数据，要加油哦!");
		return ;
	}
	for(var i=0;i<projectRankList.length;i++){
		var projectRank=projectRankList[i];
		var tr=document.createElement("tr");
		
		
		var projectNameTd=document.createElement("td");
		projectNameTd.innerHTML=projectRank.projectName;
		tr.appendChild(projectNameTd);
		
		var deptIdTd=document.createElement("td");
		deptIdTd.innerHTML = projectRank.deptName;
		tr.appendChild(deptIdTd);
		
		var projectSummaryTd=document.createElement("td");		
		projectSummaryTd.innerHTML=projectRank.projectSummary.toFixed(2);
		tr.appendChild(projectSummaryTd);
		
		var projectNumTd=document.createElement("td");
		projectNumTd.innerHTML=projectRank.projectNum;
		tr.appendChild(projectNumTd);				
		
		var avgServicedTimeTd=document.createElement("td");
		avgServicedTimeTd.innerHTML=projectRank.avgServiceTime;
		tr.appendChild(avgServicedTimeTd);
		
		var curRankTd=document.createElement("td");
		curRankTd.innerHTML=projectRank.projectRank;
		tr.appendChild(curRankTd);
		
		var lastRankTd=document.createElement("td");
		lastRankTd.innerHTML=projectRank.lastProjectRank;
		tr.appendChild(lastRankTd);
		
		tbody.appendChild(tr);
		jQuery("#projectLaborPerformanceRank tbody").remove();
		jQuery("#projectLaborPerformanceRank").append(tbody);
		
	}
	
}

function sortByNum(storeId,begin,end,dateType){
	jQuery.ajax({
		type:"post",
		data:{"storeId":storeId,"begin":begin,"end":end,"dateType":dateType},
		url:baseUrl+"laborperformance/view/laborperformance/GoodsRankByNum",
		dataType:"json",
		success:function(returnData){
...
function parseResult(projectList){
	var projectRankList=projectList;
	var tbody=document.createElement("tbody");
	if(projectRankList.length==0){
		dialog("本部门当前没有数据，要加油哦!");
...
//	window.open(baseUrl + "model/shift.xls");
	window.open(baseUrl + "importModelOfShiftInfo/action/download");
});