...
function refreshTableData(page){
	...
	for (var i = 0; i < List.length; i++) {
		...
		if(Objective.employeeObjective==null){
			...
			//现金业绩
			var cashPerformance = document.createElement("td");
			cashPerformance.innerHTML = "<input type='text' class='input80' name='cashPerformance' value='"+0+"'>";
			tr.appendChild(cashPerformance);
			...
			var cashPerformance = document.createElement("td");
			if(Objective.employeeObjective.chargeSales==null){
				cashPerformance.innerHTML = "<input type='text' name='cashPerformance' class='input80' value='"+0+"'>";
			}else{
				cashPerformance.innerHTML = "<input type='text' name='cashPerformance' class='input80' value='"+Objective.employeeObjective.cashPerformance+"'>";
			}
			tr.appendChild(cashPerformance);
...
function openedit(id){
	...
	//现金业绩
	var cashPerformance=jQuery("#"+id+"").children().find("input[name='cashPerformance']").val();
	if (isNaN(totalProjectSales)){
        dialog("现金业绩只能为数字或者小数！");
        return;
     }
	if(cashPerformance==null || cashPerformance==""){
		cashPerformance = 0;
	}
	addData["cashPerformance"] = cashPerformance;
	...
	jQuery.ajax({
		...
		success : function(e){
			if(e.msg==null){
				...
				jQuery(".f").text(e.msg.cashPerformanceSum);
