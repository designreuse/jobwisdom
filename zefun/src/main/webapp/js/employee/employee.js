...
function addsave(){
	...
	if(name==""||name==null){
		...
		addData["name"] = name;
	//员工身份证(如果身份证不为空就校验正确性)
	var identityCard = jQuery("#identityCard").val();
	if (identityCard != null && identityCard != undefined && identityCard != "" && identityCard.trim().length != 0) { 
		if(!identityCard || !/^[1-9][0-9]{5}(19[0-9]{2}|200[0-9]|2010)(0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])[0-9]{3}[0-9xX]$/i.test(identityCard)) {
	        dialog("身份证号格式错误");
	        return;
	    }
	...
	var employeeDesc = jQuery("#employeeDescOfAdd").val();
	if (employeeDesc != null && employeeDesc.length != 0) {
		addData["employeeDesc"] = employeeDesc;
	}
...
function openedit(id){
	...
	jQuery.ajax({
		...
		success : function(e){
            ...
            jQuery("#employeeDescOfEdit").val(e.msg.employeeDesc);
            //账号也需要回显
            jQuery("#updateUserName").val(e.msg.userName);
...
function updatesave(){
	...
	var employeeDesc = jQuery("#employeeDescOfEdit").val();
	if (employeeDesc != null && employeeDesc.length != 0) {
		addData["employeeDesc"] = employeeDesc;
	}
...
function deleteinfo(id){
	 ...
	 * 在EmployeeService.deleteEmployee()判断
	 */
...
//	window.open(baseUrl + "model/employee.xls");
	...
	//动态添加了下拉框
	window.open(baseUrl + "importModelOfEmployeeInfo/action/download");
});
 ...
 * 员工根据"姓名/工号/手机号"取消分页
 */
function findAllByNameOrCodeOrPhone() {
	...
	jQuery.ajax({
		...
		success : function(e){
			...
			refreshTableDataAfterListAll(e.msg);
		}
	});
}
//全查询之后刷新表格数据
