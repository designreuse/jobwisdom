...
//      canvas.height = naturalHeight;
    ...
    initCropBox: function () {
      ...
      cropBox.width = options.width ? options.width : min(max(cropBox.width, cropBox.minWidth), cropBox.maxWidth);
      cropBox.height = options.height ? options.width : min(max(cropBox.width, cropBox.minWidth), cropBox.maxWidth);
  ...
  $.extend(prototype, {
    ...
    zoomTo: function (ratio, _originalEvent) {
      ...
      if (ratio >= 0 && this.isBuilt && !this.isDisabled && options.zoomable) {
        ...
        if(canvas.width >= 3000 ||  canvas.height >= 3000){
        	dialog('最大了');
        	canvas.width = 3000;
            canvas.height = 3000;
