jQuery(function(){
    ...
    jQuery('body').delegate('.isCountLimit', 'lcs-statuschange', function() {
	    var status = (jQuery(this).is(':checked')) ? 'checked' : 'unchecked';
	    if(status == 'checked'){
	    	jQuery(this).val(1);
	    	jQuery("input[name='isCountLimit']").val(1);
	    	jQuery("input[name='projectCounts']").val('');
	    	jQuery("input[name='projectCounts']").show();
	    }else{
	    	jQuery(this).val(0);
	    	jQuery("input[name='isCountLimit']").val(0);
	    	jQuery("input[name='projectCounts']").val(999);
	...
	for(var i=0; i<comboNameObj.length; i++){
		...
		if(inputVal != ""){
			if(comboName.indexOf(inputVal) == -1 ){
				...
				var id = jQuery(comboNameObj[i]).attr("id");
				str = str + '<li class="text-center" comboId="'+id+'"><sapn>'+comboName+'</sapn></li>';
			}
		}else{
			jQuery(".fuzzysearch").hide();
		}
...
jQuery(".fuzzysearch").delegate("li","click",function(){
	...
	jQuery("#"+jQuery(this).attr("comboId")).triggerHandler("click");
	jQuery("#"+jQuery(this).attr("comboId")).addClass("active");
});
	...
	jQuery.ajax({
        ...
        success: function(data) {
            if(data.code == 0){
            	...
            	//是否有次数限制
            	var isCountLimit = comboInfo.isCountLimit;
            	if(isCountLimit == 1){
            		jQuery('#isCountLimit').lcs_on();//选中
            		jQuery("input[name='isCountLimit']").val(1);
            		for(var i=0; i<comboProjectList.length; i++){
            			var projectCount = comboProject.projectCount;
            			jQuery("input[name='projectCounts']").eq(i).val(projectCount);
            		}
            	}else{
            		jQuery('#isCountLimit').lcs_off();//关闭
            		jQuery("input[name='isCountLimit']").val(0);
            		jQuery("input[name='projectCounts']").hide();
            	}
            	//疗程商品信息
...
function addProjectList(){
	...
	jQuery("#projectSetTbody").find("select").chosen();
	if(jQuery("input[name='isCountLimit']").val() == 0){
		jQuery("input[name='projectCounts']").val(999);
		jQuery("input[name='projectCounts']").hide();
	}
}
...
function changeProjectSel(obj){
	...
	if(projectId != -1){
		...
		jQuery(obj).parent().parent().find("input[name='projectPrice']").val(0);
		jQuery(obj).parent().parent().find("span[name='projectPriceSpan']").text("0");
	}
	return;
	jQuery.ajax({
        cache: true,
...
function sortComboByDept(deptId,dateType,deptName){
	jQuery.ajax({
		type:"post",
		data:{"deptId":deptId,"dateType":dateType,"deptName":deptName},
		url:baseUrl+"packagesales/view/packagesales/rankComboByDept",
		dataType:"json",
...
function parseData(comboRankList){
	var comboList=comboRankList;
	var tbody=document.createElement("tbody");
	if(comboRankList.length==0){
	...
	for(var i=0; i<comboList.length; i++){
		...
		projectNameTd.innerHTML=combo.comboName;
		tr.appendChild(projectNameTd);
		
		var deptNameTd=document.createElement("td");
		deptNameTd.innerHTML=combo.deptName;
		tr.appendChild(deptNameTd);
		
		var saleCntTd=document.createElement("td");
		saleCntTd.innerHTML=combo.comboCnt;
		tr.appendChild(saleCntTd);
		
		var saleAmtTd=document.createElement("td");
		saleAmtTd.innerHTML=combo.comboAmount.toFixed(2);
		tr.appendChild(saleAmtTd);
				
		var curSaleRankTd=document.createElement("td");
		curSaleRankTd.innerHTML=combo.comboRank;
		tr.appendChild(curSaleRankTd);
		
		var lastSaleRankTd=document.createElement("td");
		lastSaleRankTd.innerHTML=combo.lastRank;
		tr.appendChild(lastSaleRankTd);
		
		tbody.appendChild(tr);
		
		jQuery("#packageRank tbody").remove();
		jQuery("#packageRank").append(tbody);
	}
}

function sortComboByNum(storeId,begin,end,dateType){
	jQuery.ajax({
		type:"post",
		data:{"storeId":storeId,"begin":begin,"end":end,"dateType":dateType},
		url:baseUrl+"packagesales/view/packagesales/rankComboByNumOrSales",
		success:function(returnData){
...
function parseResult(comboProject){
	var comboList=comboProject;
	var tbody=document.createElement("tbody");
	if(comboProject.length==0){
		dialog("本部门当前没有数据，要加油哦!");
		return ;
	}
	for(var i=0; i<comboList.length; i++){
