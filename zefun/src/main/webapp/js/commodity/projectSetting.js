	...
	for(var i=0; i<projectNameObj.length; i++){
		...
		if(inputVal != ""){
			if(projectName.indexOf(inputVal) == -1 ){
				...
				var id = jQuery(projectNameObj[i]).attr("id");
				str = str + '<li class="text-center" projectId="'+id+'"><sapn>'+projectName+'</sapn></li>';
			}
		}else{
			jQuery(".fuzzysearch").hide();
		}
...
jQuery(".fuzzysearch").delegate("li","click",function(){
	...
	console.log(jQuery(this).children("sapn").text());
	jQuery("#"+jQuery(this).attr("projectId")).triggerHandler("click");
	jQuery("#"+jQuery(this).attr("projectId")).addClass("active");
});
	...
	jQuery.ajax({
        ...
        success: function(data) {
            if(data.code == 0){
            	...
            	jQuery("select[name='projectType']").val(projectInfo.projectType);
            	jQuery("select[name='projectType']").trigger("liszt:updated");
            	...
            	for (var i = 0; i < jQuery("select[name='shiftMahjongId']").length; i++){
            		for(var j=0; j<projectCommissionList.length; j++){
        				...
        				if (jQuery("select[name='shiftMahjongId']").eq(i).val() == shiftMahjongId){
		            		...
		            		trEl.append("<td><input type='text' datatype='n' nullmsg='请输入指定提成!' errormsg='指定提成:请输入数字' class='input30' name='assignCash' value='"+assignCash+"'/><span class='percent-symbol'>"+assignTypeStr+"</span></td>");
		            		trEl.append("<td><input type='text' datatype='n' nullmsg='请输入非指定提成!' errormsg='非指定提成:请输入数字' class='input30' name='assignCard' value='"+assignCard+"'/><span class='percent-symbol'>"+assignTypeStr+"</span></td>");
		            		trEl.append("<td><input type='text' datatype='n' nullmsg='请输入预约奖励!' errormsg='预约奖励:请输入数字' class='input30' name='appointmentReward' value='"+appointmentReward+"'/><span class='percent-symbol'>元</span></td>");
...
function saveProjectInfo(){
	//大小项的判断
	if(jQuery("select[name='projectType']").val() == 0){
		dialog("请选择项目类型");
		return;
	}
	//对预约奖励进行判断
	...
	//对预约进行判断
	if (jQuery(".isDisableApp[value='1']").length == 0){
		dialog("轮牌步骤中为没有可预约项");
		return;
	}
	//对会员折扣进行判断
	...
	//对业绩进行判断
//	var formance = 0;
//	for (var i = 0; i < jQuery("select[name='stepPerformanceType']").length; i++) {
//		if(jQuery("select[name='stepPerformanceType']").eq(i).val() == 2){
//			formance = formance + Number(jQuery("select[name='stepPerformanceType']").eq(i).parent().next().find("input[name='stepPerformance']").val());
//			console.log(formance);
//		}
//	}
//	if(formance > jQuery("input[name='projectPrice']").val()){
//		dialog("业绩固定总值大于项目价格");
//		return;
//	}
...
function emptyInput(){
	...
	jQuery("select[name='projectType']").val(0);
	jQuery("select[name='projectType']").trigger("liszt:updated");
